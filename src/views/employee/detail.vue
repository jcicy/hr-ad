          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="部门" prop="departmentId">
                <!-- 放置及联部门组件 -->
                <!-- 将class类作用于组件中的根元素上 -->
                <select-tree v-model="userInfo.departmentId" class="select-tree" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
        ...
        correctionTime: [
          { required: true, message: '请选择转正时间', trigger: 'blur' },
          {
            validator: (rule, value, callback) => {
              // new Date() 日期对象转换
              // console.log('转正时间', value)
              // console.log('入职时间', new Date(this.userInfo.timeOfEntry))
              // console.log('转正时间 - 入职时间', value - new Date(this.userInfo.timeOfEntry))
